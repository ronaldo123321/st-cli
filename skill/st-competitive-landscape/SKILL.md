...
1) 用 **全球口径** 抓取 Sensor Tower 数据（上月 as-of，month facets）：
...
- 时间口径：使用 `data.source.month` + `data.source.as_of`（上月 as-of）。
- 每个竞品必须输出：
  - **Revenue（USD）**：`st.revenue_as_of_last_month_usd`
  - **Download counts**：`st.downloads_as_of_last_month.downloads_absolute`
  - **MAU**：`st.mau_as_of_last_month.mau_absolute`
  - **Market share**：`st.market_share_as_of_last_month.share_percent`（如缺失需标注 unavailable）
